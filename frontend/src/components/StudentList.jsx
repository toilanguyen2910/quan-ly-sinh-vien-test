...
const StudentList = ({ students, onEdit, onDelete, onViewDetail, isLoading, isAdmin }) => {
  ...
  const processedStudents = useMemo(() => {
    ...
    return result;
  }, [students, searchTerm, filterMajor, filterGpa, sortConfig]);
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === processedStudents.length) setSelectedIds([]);
    else setSelectedIds(processedStudents.map(s => s.id));
  };
  ...
  const handleExport = () => {
    const exportData = processedStudents.map(s => ({
      'Mã SV': s.studentCode, 'Họ Tên': s.fullName, 'Chuyên Ngành': s.major, 'GPA': s.gpa, 'Email': s.email, 'Số Điện Thoại': s.phone
    }));
    ...
    <div className="glass" style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <select value={filterMajor} onChange={(e) => setFilterMajor(e.target.value)} style={{ padding: '0.5rem 1rem', background: 'var(--input-bg)', border: '1px solid var(--surface-border)', borderRadius: '8px', color: 'var(--text-main)', outline: 'none' }}>
            {majors.map(m => <option key={m} value={m}>{m === 'All' ? 'Tất cả ngành' : m}</option>)}
          </select>

          <select value={filterGpa} onChange={(e) => setFilterGpa(e.target.value)} style={{ padding: '0.5rem 1rem', background: 'var(--input-bg)', border: '1px solid var(--surface-border)', borderRadius: '8px', color: 'var(--text-main)', outline: 'none' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            {isAdmin && selectedIds.length > 0 && (
              <button onClick={() => onDelete(selectedIds)} className="btn btn-primary" style={{ background: 'var(--danger)', padding: '0.5rem 1rem' }}>
        ...
        <table>
          <thead>
            <tr>
              {isAdmin && (
                <th style={{ width: '40px' }}>
                  <input type="checkbox" checked={processedStudents.length > 0 && selectedIds.length === processedStudents.length} onChange={toggleSelectAll} style={{ cursor: 'pointer' }} />
                </th>
              )}
            ...
            <AnimatePresence mode="popLayout">
              {processedStudents.length === 0 ? (
                ...
                processedStudents.map((student) => (
                  <motion.tr key={student.id} layout variants={tableRowVariants} initial="hidden" animate="visible" exit="exit" className={`clickable-row ${selectedIds.includes(student.id) ? 'selected-row' : ''}`} onClick={() => onViewDetail && onViewDetail(student)}>
                    {isAdmin && (
                      <td>
                        <input type="checkbox" checked={selectedIds.includes(student.id)} onChange={(e) => toggleSelect(e, student.id)} onClick={(e) => e.stopPropagation()} style={{ cursor: 'pointer' }} />
                      </td>
                    )}
                    <td><span className="badge">{student.studentCode}</span></td>
                    <td style={{ fontWeight: 500 }}>{student.fullName}</td>
                    <td>{student.major}</td>
                    <td><span style={{ color: student.gpa >= 3.2 ? 'var(--success)' : (student.gpa < 2.0 ? 'var(--danger)' : 'inherit'), fontWeight: 'bold' }}>{student.gpa}</span></td>
                    <td><div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{student.email}<br />{student.phone}</div></td>
                    <td>
                      <div className="actions-cell">
                        <button className="btn-icon" onClick={(e) => { e.stopPropagation(); onViewDetail && onViewDetail(student); }} title="Chi tiết"><Eye size={16} /></button>
                        {isAdmin && (
                          <>
                            <button className="btn-icon" onClick={(e) => { e.stopPropagation(); onEdit(student); }} title="Sửa"><Pencil size={16} /></button>
                            <button className="btn-icon danger" onClick={(e) => { e.stopPropagation(); onDelete(student.id); }} title="Xóa"><Trash2 size={16} /></button>
                          </>
                        )}
                      </div>
                    </td>
